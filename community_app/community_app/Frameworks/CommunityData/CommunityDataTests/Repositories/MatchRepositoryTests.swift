...
final class MatchRepositoryTests: XCTestCase {
    ...
    func testMatchDetail_WhenServerReturnsSuccess_ReturnsResponse() async throws {
        ...
        // Act
        let result = try await sut.getMatch(.init(""))

        // Assert
        XCTAssertTrue(result == expectedResponse)
    }
    
    func testMatchDetail_WhenServerReturns401_ThrowsServerError() async {
        URLProtocolMock.requestHandler = { request in
            let response = HTTPURLResponse(
                url: request.url!,
                statusCode: 401,
